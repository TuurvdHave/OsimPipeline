...
for x = 1:length(trcFilenames)
    ...
    if strcmpi(filename(end-3:end),'.trc')
        ...
        % checked here
        kinfilenames = dir(char(fullfile(path,subjectname,'OpenSim\InverseKinematics')));
        if strcmpi(filename(1:end-4),kinfilenames(a).name(1:end-4)) % if no events are detected
            finfilename = filename(1:end-4);
