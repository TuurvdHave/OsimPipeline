...
rl_Q = DSt.radius_l.Q(DSt.walking_range, :);% sit-to-stand quaternion range 
squatAngVel_GS = DSt.radius_l.g_GS(DSt.walking_range, :);% get angular velocity GS
squatAngVel_SS = DSt.radius_l.g(DSt.walking_range, :);% get angular velocity LS

figure;
subplot(2,1,2);plot(squatAngVel_GS(2:end, :));legend('X_{GS}', 'Y_{GS}', 'Z_{GS}')
title('Radius left sensor - Global System (not aligned Y-axis)');grid on;ylabel('angVel (rad/s)');
subplot(2,1,1);plot(squatAngVel_SS(2:end, :));legend('X_{LS}' ,'Y_{LS}' ,'Z_{LS}');
title('Radius left sensor - Local System');grid on;ylabel('angVel (rad/s)');
...
q_axisRot_rl = quaternion([0 0 e(3)], 'eulerd', 'XYZ', 'frame');

rl_Q_align1 = quaternion(q_axisRot).* rl_Q; 

squatAngVel_GS_align = angvel(rl_Q_align1, 1/fs, 'frame'); % get angular velocity GS
...
% disp(['rotAngle Z - radius l = ' , num2str(e(3)), ' deg']);

%%
% viewer = HelperOrientationViewer;
% for ii=1:size(rl_Q_align1)
%     viewer(rl_Q_align1(ii,:));
%     pause(0.01);
% end
