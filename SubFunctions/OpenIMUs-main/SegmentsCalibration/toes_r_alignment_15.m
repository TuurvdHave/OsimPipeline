...
car_Q = DSt.calcn_r.Q(DSt.walking_range, :);
AngVel_GS = DSt.calcn_r.g_GS(DSt.walking_range, :); % get angular velocity GS
AngVel_SS = DSt.calcn_r.g(DSt.walking_range, :);% get angular velocity LS

figure;
subplot(2,1,2);plot(AngVel_GS(2:end, :));legend('X_{GS}', 'Y_{GS}', 'Z_{GS}')
title('Walking - foot right sensor - Global System (not aligned Y-axis)');grid on;ylabel('angVel (rad/s)');
subplot(2,1,1);plot(AngVel_SS(2:end, :));legend('X_{LS}' ,'Y_{LS}' ,'Z_{LS}');
title('Walking - foot right sensor - Local System');grid on;ylabel('angVel (rad/s)');
...
q_axisRot_car = quaternion([0 0 e(3)], 'eulerd', 'XYZ', 'frame');


car_Q_align1 = quaternion(q_axisRot).* car_Q; %sit-to-stand align
car_Q_alignZ = quaternion(q_axisRot_car).* car_Q; %sit-to-stand align

AngVel_GS_alignZ = angvel(car_Q_alignZ, 1/fs, 'frame'); % get angular velocity GS
...
title('Walking - foot right sensor - Global System (aligned X-axis)');grid on;ylabel('angVel (rad/s)');
...
title('Ankle Right Flexion Rotation Axis');axis equal;xlabel('X');ylabel('Y');zlabel('Z');
...
subtalar_angle_r = sqrt(az^2+ay^2);
...
% Get acceleration, angular velocity, quaternion for the sit-to-stand in GS and LS
car_Q = DSt.calcn_r.Q(DSt.adduction_range_r, :);% sit-to-stand quaternion range 
AngVel_GS = DSt.calcn_r.g_GS(DSt.adduction_range_r, :);% get angular velocity GS
AngVel_SS = DSt.calcn_r.g(DSt.adduction_range_r, :);% get angular velocity LS
...
car_Q_alignZ = quaternion(q_axisRotZ).* car_Q; %sit-to-stand align
AngVel_GS_alignZ = angvel(car_Q_alignZ, 1/fs, 'frame'); % get angular velocity GS
AngVel_GS_alignZ_f = filtfilt3D(bLow, aLow, AngVel_GS_alignZ(2:end, :));

figure;plot(AngVel_GS_alignZ_f);legend('X_{GS}', 'Y_{GS}', 'Z_{GS}')
title('Hip AA - foot right sensor - Global System (aligned - X_{rotZ})');grid on;ylabel('angVel (rad/s)');
...
disp(['ankle_angle_r: ', num2str(ankle_angle_r), char(13),...
        'subtalar_angle_r: ', num2str(subtalar_angle_r)]);
